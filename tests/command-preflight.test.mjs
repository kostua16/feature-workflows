...
test('pipeline commands reference the project .claude/workflows only on legacy-detection lines', () => {
  for (const file of PIPELINE_COMMANDS) {
    const offenders = read(file)
      .split('\n')
      .filter((line) => /(^|[^~$}])\.claude\/workflows/.test(
        line
          .replaceAll('~/.claude/workflows', '')
          .replaceAll('$USERPROFILE/.claude/workflows', '')
...
test('setup.md is a doctor: no project-dir install, dangling-link detection present', () => {
  ...
  // The repair block must carry the same three-tier (ln -> powershell -> cp) fallback the
  // preflights embed inline — never a bare `ln -sfn` whose copy fallback is only prose.
  assert.ok(setup.includes('command -v powershell >/dev/null 2>&1 && powershell'), 'setup repair must carry the native powershell fallback tier')
  assert.ok(setup.includes('cp "${CLAUDE_PLUGIN_ROOT}/workflows/feature-pipeline.js" ~/.claude/workflows/feature-pipeline.js'), 'setup repair must carry the inline cp fallback tier')
  assert.ok(setup.includes('uname -s'), 'setup must detect the OS')
